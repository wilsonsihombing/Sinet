    <!-- navbar -->
    <div class="container-fluid">
        <nav
          class="row navbar navbar-expand-lg navbar-light bg-white navbar-custom fixed-top"
        >
          <div class="col-md-0.5 offset-md-1">
            <a href="#" class="navbar-brand">
              <img src="frontend/images/logo.png" alt="logo" />
            </a>
          </div>
          <div class="col-md-7">
            <button
              class="navbar-toggler"
              type="button"
              data-toggle="collapse"
              data-target="#navb"
            >
              <span class="navbar-toggler-icon"></span>
            </button>
  
            <div class="collapse navbar-collapse" id="navb">
              <ul class="navbar-nav ml-auto mr-3">
                <li class="nav-item mx-md-2">
                  <a href="#" class="nav-link active">HOME</a>
                </li>

                <li class="nav-item dropdown">
                  <a
                    href="#"
                    class="nav-link dropdown-toggle"
                    id="navbardrop"
                    data-toggle="dropdown"
                  >
                    PROGRAM
                  </a>
                  <div class="dropdown-menu">
                    <a href="#" class="dropdown-item">Divisi Kesenian</a>
                    <a href="#" class="dropdown-item">Divisi Humas</a>
                    <a href="#" class="dropdown-item">Divisi Kerohanian</a>
                  </div>
                </li>

                <li class="nav-item mx-md-2">
                  <a href="#" class="nav-link">EVENT</a>
                </li>

                <li class="nav-item dropdown">
                  <a
                    href="#"
                    class="nav-link dropdown-toggle"
                    id="navbardrop"
                    data-toggle="dropdown"
                  >
                    SERVICES
                  </a>
                  <div class="dropdown-menu">
                    <a href="#" class="dropdown-item">QnA</a>
                    <a href="#" class="dropdown-item">Info Karier</a>
                  </div>
                </li>

              </ul>
  
              <!-- mobile button -->
              <form class="form-inline my-lg-0 d-sm-block d-md-none">
                <div>
                  <button class="btn btn-login btn-navbar-right px-0">
                    MyHimsi
                    <img
                      src="frontend/images/user profile.png"
                      alt="user"
                      class="img-user ml-1"
                    />
                  </button>
                </div>
              </form>
  
              <!-- desktop button -->
              <form class="form-inline my-lg-0 d-none d-md-block">
                <div>
                  <button class="btn btn-login btn-navbar-right px-0">
                    MyHimsi
                    <img
                      src="frontend/images/user profile.png"
                      alt="user"
                      class="img-user ml-1"
                    />
                  </button>
                </div>
              </form>
            </div>
          </div>
        </nav>
      </div>